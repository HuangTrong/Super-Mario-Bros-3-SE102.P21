...
CKoopa::CKoopa(float x, float y, BOOLEAN block) :CGameObject(x, y) {
	...
	this->m_x = nullptr;
	this->m_y = nullptr;
	this->is_mario_holding = FALSE;
	this->is_mario_kicked = FALSE;
	this->shell_start = 0;
	this->shell_vibrate_start = 0;
	this->shell_coming_out_start = 0;
	if (block)
	{
		platformChecker = new CPlatformCheck(RelativedPositionOfPlatformChecker(x, nx), y, KOOPA_BBOX_HEIGHT, KOOPA_BBOX_WIDTH);
	}
...
void CKoopa::OnCollisionWithOtherUnit(LPCOLLISIONEVENT e)
{
	if (state == KOOPA_STATE_SHELL_MOVE)
	{
		if (dynamic_cast<CMario*>(e->obj))
		{
			...
			// Mario will handle this collision
		}
...
void CKoopa::OnCollisionWithBlock(LPCOLLISIONEVENT e)
{
	if (e->nx != 0)
	{
		...
		if (dynamic_cast<CBrick*>(e->obj))
		{
			...
			if (brick->GetState() == BRICK_STATE_IDLE && is_mario_kicked)
...
void CKoopa::Update(DWORD dt, vector<LPGAMEOBJECT>* coObjects)
{
	...
	if (isBlockByPlatform && state == KOOPA_STATE_WALKING && isOnPlatform)
	{
	...
	if (is_mario_holding && m_x != nullptr && m_y != nullptr && m_nx != nullptr)
	{
