...
TestMapScene::TestMapScene(int id, LPCWSTR filePath) :
	CScene(id, filePath) {
	key_handler = new CTestMapKeyHandler(this);
	row = 0;
	col = 0;
	origin_x = 0;
	origin_y = 0;
...
void TestMapScene::_ParseSection_BACKGROUND(string line) {
...
void TestMapScene::_ParseSection_PLAYER(string line) {
	...
	player = new CMarioTest(x, y);

}
void TestMapScene::_ParseSection_MAP_SETTINGS(string line) {
...
void TestMapScene::_ParseSection_PATH(string line) {
...
void TestMapScene::_ParseSection_PORTAL(string line) {
	vector<string> tokens = split(line);
...
void TestMapScene::Update(DWORD dt) {
	player->Update(dt);
...
void TestMapScene::ExploreNextMove(float& x, float& y, int direction) {
	//
	int ex, ey;
...
int TestMapScene::GetPortalIfStandingOn(float x, float y)
{
	int ex, ey;
	ex = x / GRID_SIZE - origin_x;
	ey = y / GRID_SIZE - origin_y;
...
void TestMapScene::SetState(int state) {
